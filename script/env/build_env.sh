...
pip install uv
uv -v pip install --system datasets evaluate scikit-learn deepspeed tensorboard optuna matplotlib peft
cd $LLAMAFACTORY_PATH
uv -v pip install --system --no-deps -e .
uv -v pip install --system datasets==3.2.0 accelerate==1.2.1 peft==0.12.0 trl==0.9.6 transformers==4.51.1
