...
cd /root/autodl-fs/MGTrainer/script/examples/functioncal
# 获取当前脚本所在目录的绝对路径
SCRIPT_DIR=$(pwd)/../../
# 构建相对路径
MILDGIRAFFE_SRC_PATH="$SCRIPT_DIR/../src/mildgiraffe_train.py"
CONFIG_PATH="$SCRIPT_DIR/../config/"
OUTPUT_DIR=/root/autodl-fs/outputs/saved_model/funcall
TRAIN_ARGES=( \
   --task_type generate \
    --train_stage sft \
    --num_train_epochs 3 \
    ...
    --data_path train_vehicle_mildgraffe_funcal \
    --deepspeed "$CONFIG_PATH/deepspeed/ds_z2_config.json" \
    --dataloader_prefetch_factor 2 \
    --template qwen_2p5 \
    --llamafactory /root/autodl-fs/LLaMA-Factory/examples/train_full/llama3_full_sft.yaml \
)
