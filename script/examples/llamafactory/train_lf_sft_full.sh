...
cd /autodl-fs/data/MGTrainer/script/examples/llamafactory
# 获取当前脚本所在目录的绝对路径
SCRIPT_DIR=$(pwd)/../../
# 构建相对路径
MILDGIRAFFE_SRC_PATH="$SCRIPT_DIR/../src/mildgiraffe_train.py"
CONFIG_PATH="$SCRIPT_DIR/../config/"
OUTPUT_DIR=/autodl-fs/data/outputs/saved_model/llamafactory/llamafactory_sft_full
TRAIN_ARGES=( \
    --task_type generate \
    --train_stage sft \
    --num_train_epochs 3 \
    ...
    --model_name_or_path /autodl-fs/data/model_public/Qwen2.5-0.5B-Instruct \
    ...
    --data_path /autodl-fs/data/MGTrainer/test/dataset/ruozhiba_qa.json \
    --deepspeed "$CONFIG_PATH/deepspeed/ds_z2_config.json" \
    --dataloader_prefetch_factor 2 \
    --template qwen \
    --llamafactory /autodl-fs/data/LLaMA-Factory/examples/train_full/llama3_full_sft.yaml \
)

# 通过管道检测存在性（推荐）
if printf '%s\n' "${TRAIN_ARGES[@]}" | grep -qx -- '--llamafactory'; then
    sh "$SCRIPT_DIR/env/build_env_llamafactory.sh"
else
